<template>
  ...
  <div class="form-marco">
    <h2 class="form-titulo">Eliminar Información del Estudiante</h2>
    <div class="del_marco">
      <h3>Está Seguro(a) que quiere Eliminar</h3>
      <h3>La información del Curso?</h3>
    </div>
    <Message
      v-if="show"
      :message="message"
    ...
    />
    <div class="botones">
      <button type="button" @click="borrarProd(courseID.value)">
        Eliminar
      </button>
...
const borrarProd = async (id) => {
  ...
  fetch(api, {
    ...
    .catch((error) => {
      console.error("Error al eliminar la información del curso", error);
    });
};

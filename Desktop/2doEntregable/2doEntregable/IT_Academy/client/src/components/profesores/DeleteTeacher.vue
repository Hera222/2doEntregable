<template>
  ...
  <div class="form-marco">
    <h2 class="form-titulo">Eliminar Informativo del Profesor</h2>
    <div class="del_marco">
      <h3>¿Está Seguro(a) que quiere Eliminar</h3>
      <h3>la información del Profesor?</h3>
    </div>
    <Message
      v-if="show"
      :message="message"
    ...
    />
    <div class="botones">
      <button type="button" @click="borrarProd(teacherID.value)">
        Eliminar
      </button>
...
const borrarProd = async (id) => {
  let api = `${url}/teacher/${teacherID.value}`;
  fetch(api, {
    method: "DELETE",
  })
    ...
    .catch((error) => {
      console.error("Error al eliminar la información del Profesor", error);
    });
};
